...
def resume_process_user_prompt(runtime_context: LLMRuntimeContext, resolution_context: ResolutionContext, root_dsl_elements: RootElements, interaction: Optional[Interaction]) -> Tuple[Optional[InteractionRequest], RootElements]:

    # def fct_resolve(dsl_elements: List[DslBase], interaction: Optional[Interaction]) -> Tuple[Optional[InteractionRequest], List[DslBase]]:
    #     while True:
    ...
    #             outcome = dsl_element.resolve(runtime_context, ResolutionContext(), interaction)
    ...
    requested_interaction, dsl_elements = root_dsl_elements.resolve(runtime_context, resolution_context, interaction) # fct_resolve(dsl_elements, interaction)
...
def process_user_prompt_text_mode(runtime_context: LLMRuntimeContext, resolution_context: ResolutionContext, prompt: str) -> List[DslBase]:
    requested_interaction, root_dsl_elements = process_user_prompt(runtime_context, resolution_context, prompt)

    print(requested_interaction, root_dsl_elements)
    ...
    while requested_interaction:
        ...
        requested_interaction, root_dsl_elements = resume_process_user_prompt(
            runtime_context=runtime_context,
            resolution_context=resolution_context,
            root_dsl_elements=root_dsl_elements,
